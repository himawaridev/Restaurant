...
navNavigationBarLi.forEach(li =>
    li.addEventListener("click", () => {
        let arr = Array.from(li.parentElement.children);
...
window.onscroll = function() {
    ...
    if (window.pageYOffset > headerSection.offsetHeight - 75) {
        ...
        logoImage.src = "../img/logo-sora.png";
    } else {
        nav.classList.remove("active");
        logoImage.src = "../img/logo-sora-white.png";
    }
...
if (pageTitle.text === "ROSA- Restaurant") {
    ...
    menuImgs.forEach(img =>
        img.addEventListener("click", function() {
            let arr = Array.from(this.parentElement.parentElement.children);
    ...
    function boxModelFun(e) {
        ...
        if (boxModel.classList.contains("active")) {
            if (
                ...
                let arr = Array.from(menuImageContainer.children);
                let active = arr.find(div => div.classList.contains("active"));
